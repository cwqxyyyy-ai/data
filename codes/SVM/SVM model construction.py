...
df = pd.read_excel(r'/地标/陈团/实验/data/landmarkDataSets.xlsx', sheet_name='数据集')
data = np.array(df.drop(['地标编号', '名称'], axis=1))
dealdata = lambdaMARTDataDeal(data)
rs = 5
train_data, test_data = dealdata.group_split(1, 5, 255, 5)
scaler = MinMaxScaler()
...
svr_linear = SVR(kernel='linear', C=20, epsilon=0.1)
svr_linear.fit(X, y)
# Accuracy evaluation of training set
y_train_pred = svr_linear.predict(X)
# print(y_train_pred)
